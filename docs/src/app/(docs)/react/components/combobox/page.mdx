...
- **Avoid for simple search widgets**: typing in Combobox only filters the predefined items—the input can't commit a value outside that set. For search widgets that accept free-form text, consider using [Autocomplete](/react/components/autocomplete) instead.
...
- **Role**: `<Combobox.Input>` renders `role="combobox"`, and `<Combobox.List>` renders `role="listbox"` (or `role="grid"` in grid layouts).
- **Virtual focus**: focus stays on the input while navigating the list. The highlighted item is communicated through `aria-activedescendant` rather than by moving DOM focus.
- **Keyboard interactions**:
  ...
  - In `multiple` mode, <kbd>ArrowLeft</kbd> / <kbd>ArrowRight</kbd> move between selection chips, and <kbd>Backspace</kbd> removes the last or highlighted chip.
- **ARIA attributes**: selected items get `aria-selected="true"`; the list gets `aria-multiselectable="true"` in `multiple` mode.
- **Labeling**: see [Usage guidelines](#usage-guidelines) above—label `<Combobox.Input>` directly, or use `<Combobox.Label>` to label `<Combobox.Trigger>` in the [input-inside-popup](#input-inside-popup) pattern. `<Combobox.ChipRemove>` needs its own `aria-label` per chip. Keep `<Combobox.Status>` and `<Combobox.Empty>` mounted—they're `aria-live="polite"` regions that announce result counts and empty states.

This pattern loosely follows the [ARIA APG Combobox pattern](https://www.w3.org/WAI/ARIA/apg/patterns/combobox/).
