...
- **Prefer using tooltips as supplementary labels only**: Tooltips should act as supplementary labels for sighted mouse and keyboard users. They are not reliably accessible to everyone else: they never open on touch devices, and screen reader users only have the tooltip content announced when the trigger receives keyboard focus. See [Alternatives to tooltips](#alternatives-to-tooltips) for more details.
- **Provide an accessible name for the trigger**: A tooltip is not a replacement for labeling the trigger, which must always have an accessible name of its own—use `aria-label` for icon-only triggers. While the tooltip is open, the trigger is described by the tooltip content via `aria-describedby`, announced after the trigger's name on keyboard focus. If the tooltip text is identical to the name, browsers suppress the duplicate description; if it adds detail beyond the name, that detail is announced as the description.
...
- **Role**: `<Tooltip.Popup>` renders `role="tooltip"`.
- **Accessible description**: while open, `<Tooltip.Trigger>` references the popup through `aria-describedby`, so its content is announced as a description right after the trigger's own accessible name.
- **Keyboard interactions**:
  - <kbd>Tab</kbd> — moving focus onto the trigger opens the tooltip; moving focus away from it
    closes the tooltip.
  - <kbd>Escape</kbd> — dismisses the open tooltip.
- **Focus management**: the tooltip itself never receives focus. It opens on hover or on keyboard focus of the trigger and closes on blur, so it never interrupts the tab sequence.
- **Labeling the trigger**: the trigger must always carry its own accessible name—visible text, or `aria-label` for icon-only triggers. The tooltip is a supplementary description via `aria-describedby`, not a replacement for that name. See [Usage guidelines](#usage-guidelines) above.

This pattern loosely follows the [ARIA APG Tooltip pattern](https://www.w3.org/WAI/ARIA/apg/patterns/tooltip/).
